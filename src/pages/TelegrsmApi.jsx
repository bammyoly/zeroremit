...
export default function TelegramApi() {
  ...
  const refreshStatus = useCallback(async () => {
    ...
    try {
      ...
      // Seed prefs from backend response, fall back to defaults per alert type
      const initial = {};
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      try {
        ...
        // Silent — don't show error while polling
      }
    }, 4_000);
  ...
  const handleGenerateCode = async () => {
    ...
    try {
      ...
      setCode(c);
      // expiresAt comes from backend as a string (BigInt serialized)
  ...
  const handleTogglePref = async (id, value) => {
    ...
    setSavingPrefs(true);
    try {
      // ✅ Fixed payload — backend expects { wallet, prefs: { ... } }
      await apiUpdateTelegramPrefs(address, { ...prefs, [id]: value });
    } catch (e) {
      setError(e.message);
      // Revert on error
      setPrefs(p => ({ ...p, [id]: !value }));
  ...
  const deepLink = code && !codeExpired
      ...
      <section className="relative pt-24 pb-10 px-4 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <div className="flex items-center gap-2 text-xs text-zinc-500 mb-5">
            ...
            <span>›</span>
            <span className="text-zinc-300">Telegram Integration</span>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-20">
        ...
        {error && (
          <div className="mb-6 flex items-start gap-2.5 px-4 py-3 bg-red-950/40
            ...
            <button onClick={() => setError('')}
              className="ml-auto text-red-300 hover:text-red-200">×</button>
          </div>
        )}
        ...
        {tab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="flex items-start gap-3 px-5 py-4 bg-sky-500/5
              border border-sky-500/10 rounded-xl">
              <div className="w-8 h-8 rounded-lg bg-sky-500/10 flex items-center
            ...
            {!isConnected ? (
              <Card>
                <CardBody className="pt-10 pb-10 text-center">
                  <div className="w-14 h-14 rounded-2xl bg-zinc-800 flex items-center
                    justify-center mx-auto mb-4">
                    <svg className="w-7 h-7 text-zinc-500" fill="none"
                      stroke="currentColor" viewBox="0 0 24 24">
              ...
              <Card>
                <CardHeader
                  ...
                  subtitle={`Linked to @${linkStatus.username || 'user'} ${
                    linkStatus.linkedAt
                      ? '· ' + new Date(Number(linkStatus.linkedAt)).toLocaleDateString('en', {
                ...
                <CardBody className="pt-2 space-y-5">
                  ...
                  <div className="flex gap-2">
                    <a
                      ...
                      className="flex-1 h-11 flex items-center justify-center gap-2
                        bg-sky-600 hover:bg-sky-500 text-white font-medium rounded-xl
                        text-sm transition-all shadow-md shadow-sky-500/20"
                    >
                    ...
                    <button
                      ...
                      className="px-5 h-11 bg-zinc-800 hover:bg-red-950/40
                        hover:text-red-400 text-zinc-300 font-medium rounded-xl
                        text-sm transition-all border border-zinc-700"
                    >
                      Disconnect
                    </button>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 px-4 py-3 bg-emerald-500/5
                    ...
                    <span>
                      ...
                      tab to customize what triggers notifications.
                    </span>
                  </div>
                </CardBody>
              </Card>
              ...
              <>
                  ...
                  <CardBody className="pt-2 space-y-5">
                    ...
                    {!code ? (
                      ...
                      <div className="space-y-4">
                        ...
                        {/* Expired state */}
                        {codeExpired ? (
                          <div className="px-4 py-3 bg-amber-950/30 border border-amber-900/30
                            rounded-xl text-xs text-amber-400 text-center">
                          ...
                          <>
                            ...
                            <div>
                              ...
                              <div className="flex gap-2">
                                <div className="flex-1 h-12 px-4 flex items-center
                                  bg-zinc-950 border border-zinc-800 rounded-xl font-mono
                                  text-base text-sky-300 tracking-wider select-all">
                                  {code}
                                </div>
                            ...
                            {/* QR code */}
                            <div className="flex items-center gap-4 p-4 bg-zinc-950
                              ...
                              <div className="flex-1 text-xs text-zinc-500 leading-relaxed">
                                ...
                                <p>
                                  Open Telegram on your phone, scan this QR,
                                  and tap <span className="text-zinc-300">Start</span>.
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2 px-4 py-3
                              ...
                              <svg className="w-4 h-4 animate-spin text-sky-400"
                        ...
                        <button
                          onClick={handleGenerateCode}
                          disabled={generating}
                          className="w-full h-10 text-xs font-medium text-zinc-400
                            hover:text-zinc-200 bg-zinc-900 hover:bg-zinc-800
                            border border-zinc-800 rounded-lg transition-all
                            disabled:opacity-40"
                        >
                ...
                <Card>
                  <CardHeader
                    ...
                    subtitle="If the deep link doesn't open Telegram"
                  />
                  <CardBody className="pt-2 space-y-2.5 text-xs text-zinc-400 leading-relaxed">
                    {[
                      <>Open Telegram and search for <span className="text-zinc-200 font-mono">@{BOT_USERNAME}</span>.</>,
                      <>Tap <span className="text-zinc-200">Start</span>.</>,
                      <>Send: <span className="text-zinc-200 font-mono bg-zinc-900 px-1.5 py-0.5 rounded">/link {code || '<code>'}</span></>,
                    ].map((step, i) => (
                      <div key={i} className="flex gap-3">
                        <div className="w-5 h-5 rounded-full bg-zinc-800 text-zinc-400
          ...
          <div className="space-y-6">
                      ...
                      {grouped[cat].map(c => (
                        <div key={c.cmd}
                          className="group flex flex-col sm:flex-row sm:items-center
                            gap-3 p-3.5 bg-zinc-950/50 rounded-xl border border-zinc-800/40
            ...
            <div className="flex items-start gap-3 px-5 py-4 bg-violet-500/5
              ...
              <div className="w-8 h-8 rounded-lg bg-violet-500/10 flex items-center
                justify-center flex-shrink-0">
                <svg className="w-4 h-4 text-violet-400" fill="none"
                  stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
            ...
            {!isLinked ? (
              <Card>
                <CardBody className="pt-10 pb-10 text-center">
                  ...
                  <p className="text-sm text-zinc-500 max-w-sm mx-auto mb-5">
                    You need to connect your wallet to a Telegram account before
              ...
              <>
                  ...
                  <CardBody className="pt-2 space-y-2">
                    {ALERT_TYPES.map(a => (
                      <div key={a.id}
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-medium text-zinc-100 mb-0.5">{a.label}</p>
                ...
                <Card>
                  ...
                  />
                  <CardBody className="pt-2">
                    <div className="flex items-start gap-3 p-4 bg-zinc-950 rounded-xl
                      ...
                      <div className="flex-1">
                        ...
                        <div className="bg-zinc-900 rounded-2xl rounded-tl-md p-3.5
                          ...
                          <div className="flex gap-2 mt-3">
                            <button className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700
                              rounded-lg text-xs font-medium text-zinc-200 transition-colors">
                              Open in app
                            </button>
                            <button className="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700
                              rounded-lg text-xs font-medium text-zinc-200 transition-colors">
                ...
                <div className="flex items-start gap-3 px-5 py-4 bg-amber-500/5
                  ...
                  <div>
                    ...
                    <p className="text-xs text-zinc-500 leading-relaxed">
                      Use <code className="text-zinc-400 font-mono">/mute 8h</code> in the
                      bot to silence notifications for a set duration. Send{' '}
                      <code className="text-zinc-400 font-mono">/alerts off</code> to disable
                      all alerts entirely.
                    </p>
                  </div>
                </div>
